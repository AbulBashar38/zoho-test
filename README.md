...
Linting and formatting are handled by [Biome](https://biomejs.dev) (one tool in place of ESLint + Prettier), configured in `biome.json`:

```bash
npm run check       # lint + format + import-order check, no writes — run this before committing
npm run check:fix   # same, but apply every safe fix
npm run lint        # lint only
npm run lint:fix    # lint only, applying safe fixes
npm run format      # rewrite files to the canonical format
npm run ci          # non-writing check for CI; exits non-zero on any finding
```

Install the [Biome VS Code extension](https://marketplace.visualstudio.com/items?itemName=biomejs.biome) for format-on-save. `.vscode/` is gitignored, so the settings that enable it are local to your machine.
