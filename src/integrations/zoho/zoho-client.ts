...
zohoClient.interceptors.request.use(async (request) => {
...
zohoClient.interceptors.response.use(
    ...
    async (error) => {
        ...
        if (error.response?.status === 401 && request && !request._retriedAfter401) {
            ...
            return zohoClient(request)
        }

        throw toZohoApiError(error.response?.status, error.response?.data, request, error.message)
    },
)
